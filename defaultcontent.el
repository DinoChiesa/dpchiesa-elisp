...
;; and it is ready.  Try to open an empty file, foobar.el file for example.
;;
;; Template files
;;
;; Set the directory like so:
;;   (setq dc-auto-insert-directory "c:/users/dino/elisp/defaultcontent")
;;
;; Then, set the alist. This is a list of cons cells, associating
;; regexi to template files. defaultcontent checks the regexi against
;; the filename being opened and when it finds a match, it uses the
;; specified template.
;;
...
;;            ....
;;
;; In each template file, you may specify any text, plus "expandos".  The
;; expandos are uppercase words surrounded by a pair of @. At insertion time,
;; they are expanded into something different.  The known expandos are:
;;
;;  @FILENAME@  - the name of the file being filled, including dir path.
;;  @BASEFILENAME@ - the name of the file without path
;;  @BASEFILENAMELESSEXTENSION@ - filename without path or extension
;;  @DATE@
;;  @HOST@ - name of computer on which the file was created
;;  @AUTHOR@ - result of getenv("USER") or getenv("USERNAME")
;;  @ENV(xx)@ - environment variable XX
;;  @INSERT(yy)@ - insert file yy at this spot
;;  @LISP(forms)@ - eval arbitrary lisp forms
;;  @DOT@ - special meaning - place the "dot" at this place in the file

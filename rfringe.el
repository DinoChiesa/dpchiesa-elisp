;;; rfringe.el --- indicate relative locations in the fringe
;;
;; Author     : Dino Chiesa <dpchiesa@hotmail.com>
;; Created    : April 2011
;; Version    : 1.0.1
;; Keywords   : fringe, bitmap
;; URL        : http://www.emacswiki.org/emacs/RFringe
;; X-URL      : http://www.emacswiki.org/emacs/rfringe.el
;; Last-saved : <2012-March-28 21:50:17>
...
;; for example, the location of compiler error messages. Lots of "modern"
;; developer tools use the fringe or scrollbar area for this purpose.
;;
...
;; scrollbar itself. In emacs, the position and size of the scrollbar
;; indicates to the user the position and size of the current window
;; relative to the entire buffer. This is information that cound not be
;; easily or appropriately conveyed within the visual text. The fringe
;; is perfectly suited for this purpose. Users are now accustomed to
;; this convention, because it is commonly used in other applications.
;;
...
;;
;;    As you scroll through the buffer, the indicators in the fringe
;;    remain fixed, because the relative positions of those items
;;    does not change.
;;
...
;;    but it is a good example. You can turn off the flymake stuff with
;;
;;       (setq rfringe-want-flymake-magic nil)
;;
;;    You could also integrate rfringe with a diff mode, or a fixme mode,
;;    a spell-check mode, and so on. Lots of possibilities.
;;


;; Revisions:
;; 1.0  5-April-2011  Initial version
;;
;; 1.0a 5-April-2011  bug fix
;;    new advice (rfringe-remove-flymake-indicators) to remove rfringe
;;    indicators when flymake removes its overlays
;;
;;    also, make the flymake integraiton customizable.
;;


(require 'fringe)

(defcustom rfringe-want-flymake-magic t
  "Whether you want fringe indicators for flymake errors & warnings"
  :type 'boolean
  :group 'rfringe)
...
(make-variable-buffer-local 'rfringe-want-flymake-magic)
...
(defadvice flymake-delete-own-overlays (after
                                        rfringe-remove-flymake-indicators
                                        activate compile)
  (if rfringe-want-flymake-magic
      (rfringe-remove-managed-indicators)))

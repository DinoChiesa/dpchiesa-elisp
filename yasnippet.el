...
(defun yas/files-in-dir (directory)
  "Return list of plain files within DIRECTORY."
  (yas/subdirs directory t))


(defun yas/subdirs (directory &optional filep)
  "Return subdirs or files of DIRECTORY according to FILEP."
  (remove-if (lambda (file)
               (let ((fnnd (file-name-nondirectory file)))
               (or (string-match "^\\." fnnd)
                   (string-match "^#.*#$" fnnd)
                   (string-match "~$" fnnd)
                   (and filep (file-directory-p file))
                   (and (not filep) (not (file-directory-p file))))))
  ...
  (unless (file-exists-p (concat directory "/" ".yas-skip"))
    ...
    (if (and (not no-compiled-snippets)
      ...
      (let* ((default-directory directory)
        ...
        ;;
        (with-temp-buffer
          (dolist (file (yas/files-in-dir directory))
